# Title : Pearson's correlation with FDR correction SwimUp
...
INPUT_FILES = [
    ...
    # Note: this file has C2. The script will try to pair C1 with O1. If C2 is a different baseline, you may need to adjust.
...
# Regex to extract SampleID from filename, e.g., 'M8_C1', 'M11_O1'
# This pattern is more robust for the M#_C#/O# formats
sample_id_pattern = re.compile(r'GRCh37_(M\d+_[CO]\d+)\.dedup')  # Updated regex
...
def load_and_process_scores(filepath, phenocode_descriptions_map):
    ...
    try:
        ...
        if not df.empty:
            ...
            # print(f"DEBUG: Sample {sample_id} - Last 2 Phenotype Keys from score file:") # Suppress for brevity unless needed
            # print(df['Phenotype_Key_For_Lookup'].tail(2).tolist())
...
def calculate_change_scores(all_raw_data_long):
    ...
    for donor_base in df_pivot_revised['donor_base_id'].unique():
        ...
        if followup_timepoint in donor_df.columns and not donor_df[followup_timepoint].isnull().all():
            o1_scores = donor_df[followup_timepoint]
            ...
            for baseline_col in baseline_timepoints:
                if baseline_col in donor_df.columns and not donor_df[baseline_col].isnull().all():
                    c_score = donor_df[baseline_col]
                    c_col = baseline_col
                    break  # Found a baseline, use it

            if c_score is not None:
                # Calculate change score where both baseline and O1 are present
                temp_df = donor_df[['donor_base_id', 'Phenotype_Label']].copy()
                # Ensure the lengths match for subtraction, handle potential NaNs from merge
                # This operation will produce NaNs where either c_score or o1_scores is NaN
                temp_df['Change_Score'] = o1_scores.reset_index(drop=True) - c_score.reset_index(drop=True)
                temp_df['C_Score'] = c_score.reset_index(drop=True)  # Store the actual C1 or C2 score used
                temp_df['O_Score'] = o1_scores.reset_index(drop=True)
                temp_df['Baseline_Timepoint_Used'] = c_col  # For debugging/record-keeping
    ...
    # IMPORTANT CHECK BEFORE DROPNAN
    if merged_scores.empty:
        print("  - Resulting 'merged_scores' DataFrame is empty. No change scores calculated.")
        return pd.DataFrame()  # Return empty DataFrame if no data

    if 'Change_Score' not in merged_scores.columns:
        print(
            "  - Error: 'Change_Score' column not found in 'merged_scores'. This means no successful pairings were made.")
        print(f"  - merged_scores columns: {merged_scores.columns.tolist()}")
        return pd.DataFrame()  # Return empty DataFrame if column not created

    df_paired_scores = merged_scores.dropna(subset=['Change_Score'])
    df_paired_scores.rename(columns={'donor_base_id': 'donor_id'}, inplace=True)  # Rename to generic donor_id
...
if __name__ == "__main__":
    ...
    plot_significant_heatmap(R_matrix, df_significant_fdr, os.path.join(INPUT_DIR, OUTPUT_SIGNIFICANT_HEATMAP_R),
                             vmin_custom=0.95, vmax_custom=1.0,
                             cmap_base_name='Reds', n_colors_discrete=20)
